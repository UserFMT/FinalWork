...
def test_get_tow_way(code_city, data, base_url, res):
    ...
    with (allure.step('Количество элементов в '
                      'списке полученных билетов >0 ')):
        assert len(response["prices"]) > res, ('Список '
                                               'билетов по заданному '
                                               'маршуруту не найден')


@allure.title("Запрос на получение списка горячих билетов")
@allure.description('Проверяем что список горячих билетов не пустой и '
                    'есть билеты заданной авиакомпании')
...
def test_hot_tickets(code_city, code_air, base_url, status, res):
    ...
    with allure.step('Фиксируем первичные данные для поиска '):
        params = {
            ...
            "query": '\nquery HotOffersV1(\n $input: HotOffersV1Input!, \n '
                     ...
                     ' {\n offers {\n price {\n ...'
                     'riceWithDestinationCityIataFields\n }\n '
                     'old_price {\n  value\n } \n  }\n cities {\n '
                     ' ...citiesFields\n  '
                     '}\n airlines {\n  ...airlinesFields\n  }\n'
                     ' airports {\n ...airportsFields\n '
                     '}\n countries {\n ...countriesFields\n }\n }\n  '
                     '}\n\nfragment priceWithDestinationCityIataFields '
                     'on Price {\n '
                     '...priceFields\n  destination_city_iata\n}\n\nfragment '
                     'priceFields on Price '
                     '{\n depart_date\n return_date\n value\n cashback\n '
                     'found_at\n signature\n  ticket_link\n  currency\n '
                     ' provider\n with_baggage\n '
                     'segments {\n  transfers {\n  duration_seconds\n '
    ...
    with allure.step(f'В списке присутсвуют билеты авиакомпании'
                     f' c кодом {code_air}'):
        assert test_air is res


@allure.title("Запрос на получение информации о городе, "
              "установленному по дефолту")
@allure.description('Проверяем соответствие названия города '
                    'вылета установенному значению')
@allure.severity("medium")
@pytest.mark.positive_test
@pytest.mark.parametrize(
    'code_city, base_url, status',
    [('Барнаул',
...
def test_city_default(code_city, base_url, status):
    ...
    with allure.step('Статус исполнения запроса = 200'):
        assert response.status_code == status
    with allure.step(f'Проверка соответствия города по дефолту '
                     f'заданному -  {code_city}'):
        assert res[0]["name"] == code_city, ('Город по умолчанию не '
                                             'сооответствует заданному')


@allure.title("Запрос на получение авиарейсов с  идентичными "
              "городами вылета и прилета")
@allure.description('Проверяем наличие ошибки при исполнении запроса '
                    'с невалидными данными по выбранным городам')
@allure.severity("critical")
@pytest.mark.negative_test
@pytest.mark.parametrize('code_city, data, '
                         'base_url, status',
...
def test_search_parameters_negative(code_city, data, base_url, status):
    with allure.step(f'Фиксируем первичные невалидные данные для '
                     f'поиска по кодам городов маршрута : {code_city}'):
        apitest = apiPage(base_url)
    with allure.step('Выполняем запрос'):
...
def test_page_info(page_title, base_url, status):
    ...
    with allure.step(f'Статус исполнения запроса = {status}'):
        assert response.status_code == status
    with allure.step(f'Название страницы соответвтует'
                     f' заявленной - {page_title}'):
